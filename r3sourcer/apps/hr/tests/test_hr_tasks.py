...
class TestVOTasks:
    ...
    def test_send_job_offer_sms(self, mock_sms_service, job_offer):
        hr_tasks.send_job_offer_sms(job_offer, 'tpl')
    ...
    def test_send_job_offer_sms_action_sent(self, mock_sms_service, job_offer, fake_sms):
        ...
        hr_tasks.send_job_offer_sms(job_offer, 'tpl', action_sent='offer_sent_by_sms')
    ...
    def test_send_or_schedule_job_offer_sms(self, mock_send_sms, mock_jo_ts_unset, job_offer):
        hr_tasks.send_or_schedule_job_offer_sms(
    ...
    def test_send_or_schedule_job_offer_sms_rescheduled(self, mock_send_sms, mock_jo_ts_unset, job_offer):
        ...
        hr_tasks.send_or_schedule_job_offer_sms(job_offer.id, task_mock, action_sent='offer_sent_by_sms')
    ...
    def test_send_or_schedule_job_offer_sms_no_task(self, mock_send_sms, mock_jo_ts_unset, job_offer):
        hr_tasks.send_or_schedule_job_offer_sms(job_offer.id, action_sent='offer_sent_by_sms')
    ...
    def test_send_or_schedule_job_offer_no_jo(self, mock_logger):
        hr_tasks.send_or_schedule_job_offer_sms(uuid.uuid4(), action_sent='offer_sent_by_sms')

        assert mock_logger.error.called

    @mock.patch('r3sourcer.apps.hr.tasks.logger', new_callable=mock.PropertyMock())
    def test_send_or_schedule_job_offer_already_accepted(self, mock_logger, accepted_jo):
        hr_tasks.send_or_schedule_job_offer_sms(accepted_jo.id, action_sent='offer_sent_by_sms')
    ...
    def test_send_jo_confirmation_sms(self, mock_send, job_offer):
        hr_tasks.send_jo_confirmation_sms(job_offer.id)
    ...
    def test_send_recurring_jo_confirmation_sms(self, mock_send, job_offer):
        hr_tasks.send_recurring_jo_confirmation_sms(job_offer.id)
