...
class TestCoefficientRates:
    ...
    def add_rule(self, industry_rate_coefficient, rule):
        DynamicCoefficientRule.objects.create(
            rate_coefficient=industry_rate_coefficient.rate_coefficient,
            rule=rule,
        )
    ...
    def test_calc_monday_rule(self, settings, industry_rate_coefficient,
                              monday_rule):
        settings.TIME_ZONE = 'UTC'

        self.add_rule(industry_rate_coefficient, monday_rule)
        res = self.calc_res()

        assert len(res) == 1
        coeff = industry_rate_coefficient.rate_coefficient
        assert res[0]['coefficient'] == coeff
        assert res[0]['hours'] == timedelta(hours=8)

    @freeze_time(datetime(2017, 1, 1, 8, 30))
    def test_calc_not_applicable(self, settings, industry_rate_coefficient,
                                 monday_rule, industry):
        settings.TIME_ZONE = 'UTC'
    ...
    @freeze_time(datetime(2017, 1, 2, 8, 30))
    def test_calc_overtime(self, settings, industry_rate_coefficient,
                           overtime_rule):
        settings.TIME_ZONE = 'UTC'

        self.add_rule(industry_rate_coefficient, overtime_rule)
        res = self.calc_res()
    ...
    def test_calc_overtime_not_applied(
            self, settings, industry_rate_coefficient, overtime_rule):
        settings.TIME_ZONE = 'UTC'
    ...
    @freeze_time(datetime(2017, 1, 2, 8, 30))
    def test_calc_overtime_for_monday(
            self, settings, industry_rate_coefficient, monday_rule,
            overtime_rule):
        settings.TIME_ZONE = 'UTC'
    ...
    def test_calc_overtime_for_time_monday(
            self, settings, industry_rate_coefficient, monday_rule,
            overtime_rule, day_time_rule):
        settings.TIME_ZONE = 'UTC'

        self.add_rule(industry_rate_coefficient, overtime_rule)
        self.add_rule(industry_rate_coefficient, monday_rule)
        self.add_rule(industry_rate_coefficient, day_time_rule)
        res = self.calc_res()
    ...
    def test_calc_for_time_monday(self, settings, industry_rate_coefficient,
                                  monday_rule, day_time_rule):
        settings.TIME_ZONE = 'UTC'
    ...
    @freeze_time(datetime(2017, 1, 2, 8, 30))
    def test_calc_allowance(self, settings, industry_rate_coefficient,
                            allowance_rule):
        settings.TIME_ZONE = 'UTC'

        self.add_rule(industry_rate_coefficient, allowance_rule)
        res = self.calc_res()
    ...
    @freeze_time(datetime(2017, 1, 2, 8, 30))
    def test_calc_allowance_for_time(self, settings, industry_rate_coefficient,
                                     allowance_rule, day_time_rule):
        settings.TIME_ZONE = 'UTC'
    ...
    @freeze_time(datetime(2017, 1, 2, 9, 30))
    def test_calc_allowance_for_night_time(
            self, settings, industry_rate_coefficient, overtime_rule,
            night_time_rule):
        settings.TIME_ZONE = 'UTC'

        self.add_rule(industry_rate_coefficient, overtime_rule)
        self.add_rule(industry_rate_coefficient, night_time_rule)
        res = self.calc_res()
    ...
    @freeze_time(datetime(2017, 1, 2, 8, 30))
    def test_calc_two_coeff(self, settings, industry_rate_coefficient,
                            industry_rate_coefficient_another, overtime_rule,
                            monday_rule, allowance_rule):
        settings.TIME_ZONE = 'UTC'

        self.add_rule(industry_rate_coefficient, allowance_rule)
        self.add_rule(industry_rate_coefficient, monday_rule)

        self.add_rule(industry_rate_coefficient_another, overtime_rule)
        self.add_rule(industry_rate_coefficient_another, monday_rule)
        res = self.calc_res()

        assert len(res) == 3
        coeff = industry_rate_coefficient.rate_coefficient
        coeff_another = industry_rate_coefficient_another.rate_coefficient
        assert res[0]['coefficient'] == coeff
        assert res[0]['hours'] == timedelta(hours=1)
        assert res[1]['coefficient'] == coeff_another
    ...
    def test_calc_two_coeff_night(
            self, settings, industry_rate_coefficient, overtime_rule,
            industry_rate_coefficient_another, overtime_4_rule, monday_rule,
            night_time_rule, day_time_rule):
        settings.TIME_ZONE = 'UTC'

        self.add_rule(industry_rate_coefficient_another, night_time_rule)
        self.add_rule(industry_rate_coefficient_another, overtime_4_rule)
        self.add_rule(industry_rate_coefficient_another, monday_rule)
        ...
        coeff = industry_rate_coefficient.rate_coefficient
        coeff_another = industry_rate_coefficient_another.rate_coefficient
        assert res[0]['coefficient'] == coeff_another
        assert res[0]['hours'] == timedelta(hours=1, minutes=30)
        assert res[1]['coefficient'] == coeff
        assert res[1]['hours'] == timedelta(hours=1)
        assert res[2]['coefficient'] == 'base'
    ...
    def test_calc_two_coeff_night_not_applied(
            self, settings, industry_rate_coefficient, overtime_rule,
            industry_rate_coefficient_another, monday_rule, night_time_rule,
            day_time_rule):
        settings.TIME_ZONE = 'UTC'
...
class TestPriceListCoefficient(TestCoefficientRates):
    ...
    def test_get_rate_coefficients_for_company(
            self, industry, company, price_list_rate_coefficient,
            industry_rate_coefficient_company, price_list_rate, skill):
    ...
    def test_calc_company(self, industry, company, price_list_rate_coefficient,
        ...
        self.add_rule(price_list_rate_coefficient, monday_rule)
